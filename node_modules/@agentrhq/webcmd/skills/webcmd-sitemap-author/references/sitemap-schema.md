...
source: webcmd --session <session-id> browser snapshot --snapshot-mode tree | trace:<path> | adapter:<site>/<command>
---
```
...
status: verified
verified_at: YYYY-MM-DD
source: webcmd --session <session-id> browser snapshot --snapshot-mode tree
url_patterns:
  - https://github.com/*/*
state_signature:
  - selector: main
...
next: rerun `webcmd --session <session-id> browser run --stdin` against the current URL
```
