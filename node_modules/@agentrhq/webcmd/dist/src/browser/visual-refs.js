 ...
 * `data-webcmd-ref` attributes produced by the DOM snapshot path so the
 * screenshot labels map back to normal `browser click <ref>` targets.
 */
const OVERLAY_ID = '__webcmd_visual_ref_overlay';
export function installVisualRefOverlayJs(opts = {}) {
    ...
    (() => {
      ...
      const refs = Array.from(document.querySelectorAll('[data-webcmd-ref]'))
        .map((el) => {
          const rawRef = el.getAttribute('data-webcmd-ref') || '';
