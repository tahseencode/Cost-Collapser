...
const clockOrigin = clock?.timeOrigin ?? Date.now();
const clockNow = () => clock?.now() ?? Date.now() - clockOrigin;
let _timeOrigin = clockOrigin;
let _timeShift = 0;
export function setTimeOrigin(origin) {
    _timeOrigin = origin;
    _timeShift = clockOrigin - origin;
}
export function timeOrigin() {
    return _timeOrigin;
}
export function monotonicTime() {
    return Math.floor((clockNow() + _timeShift) * 1000) / 1000;
}
