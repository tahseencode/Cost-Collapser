...
export function analyzeSite(signals, registry) {
    ...
    if (antiBot.detected) {
        ...
        next = 'Read the SSR global with `webcmd --session <session-id> browser run --stdin` and page.evaluate(() => window.__INITIAL_STATE__ ?? window.__NUXT__ ?? window.__NEXT_DATA__ ?? window.__APOLLO_STATE__) — no API needed.';
    }
    else if (pattern.pattern === 'C') {
        next = 'No API visible — use `webcmd --session <session-id> browser snapshot --snapshot-mode read` or DOM extraction inside `browser run` against the rendered page.';
    }
        ...
        next = 'No strong signal. Use `webcmd --session <session-id> browser run --stdin` with response listeners and pick a pattern from the captured evidence.';
    }
    return {
