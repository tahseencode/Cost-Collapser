/**
 * Verify fixture: structural expectations for `webcmd browser verify` output.
 *
 * The adapter-author skill runbook says every published adapter must write a
 * fixture under `~/.webcmd/sites/<site>/verify/<command>.json` so later verify
...
export function validateFixture(value) {
    if (!isRecord(value))
        throw fixtureError('fixture');
    if ('args' in value && !isRecord(value.args) && !Array.isArray(value.args))
        throw fixtureError('args');
    if (!('expect' in value))
        return;
    if (!isRecord(value.expect))
        throw fixtureError('expect');
    const expect = value.expect;
    if ('columns' in expect && !stringArray(expect.columns))
        throw fixtureError('columns');
    if ('notEmpty' in expect && !stringArray(expect.notEmpty))
        throw fixtureError('notEmpty');
    if ('mustBeTruthy' in expect && !stringArray(expect.mustBeTruthy))
        throw fixtureError('mustBeTruthy');
    if ('rowCount' in expect) {
        if (!isRecord(expect.rowCount))
            throw fixtureError('rowCount');
        const { min, max } = expect.rowCount;
        if ((min !== undefined && typeof min !== 'number') || (max !== undefined && typeof max !== 'number') || (typeof min === 'number' && typeof max === 'number' && min > max))
            throw fixtureError('rowCount');
    }
...
export function fixturePath(site, command) {
    return path.join(os.homedir(), '.webcmd', 'sites', site, 'verify', `${command}.json`);
}
...
function validateStringRecord(value, name, valid) {
    if (value === undefined)
        return;
    if (!isRecord(value))
        throw fixtureError(name);
    for (const [key, item] of Object.entries(value))
        if (typeof item !== 'string' || !valid(item))
            throw fixtureError(`${name}.${key}`);
}
function fixtureError(field) {
