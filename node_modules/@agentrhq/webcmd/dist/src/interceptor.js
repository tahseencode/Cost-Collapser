 ...
 * Captured responses are pushed to `window.__webcmd_intercepted`.
 *
 * @param patternExpr - JS expression resolving to a URL substring to match (e.g. a JSON.stringify'd string)
 * @param opts.arrayName - Global array name for captured data (default: '__webcmd_intercepted')
 * @param opts.patchGuard - Global boolean name to prevent double-patching (default: '__webcmd_interceptor_patched')
 */
export function generateInterceptorJs(patternExpr, opts = {}) {
    const arr = opts.arrayName ?? '__webcmd_intercepted';
    const guard = opts.patchGuard ?? '__webcmd_interceptor_patched';
 ...
 */
export function generateReadInterceptedJs(arrayName = '__webcmd_intercepted') {
    return `
    () => {
