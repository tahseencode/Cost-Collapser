...
function toManifestEntry(cmd, adapterPath) {
    return {
        site: cmd.site,
        name: cmd.name,
        aliases: cmd.aliases,
 ...
 */
export async function loadManifestEntries(filePath, site, importer = moduleHref => import(moduleHref), clisDir = LEGACY_CLIS_DIR) {
    let src;
    try {
 ...
 */
export async function coreCommandEntries(importer = moduleHref => import(moduleHref)) {
...
export async function buildManifest() {
    ...
    for (const entry of core)
        entriesByCommand.set(`${entry.site}/${entry.name}`, entry);
    const entries = [...entriesByCommand.values()].sort((a, b) => a.site.localeCompare(b.site) || a.name.localeCompare(b.name));
