...
export async function launchDetachedApp(executable, args, label) {
    await new Promise((resolve, reject) => {
        ...
        const onError = (err) => {
            if (err.code === 'ENOENT') {
                reject(new CommandExecutionError(`Could not launch ${label}: executable not found at ${executable}`, `Install ${label}, reinstall it, or register a custom app path in ~/.webcmd/apps.yaml`));
                return;
            }
...
export async function launchElectronApp(appPath, app, args, label) {
    ...
    if (executables.length > 1) {
        throw new CommandExecutionError(`Could not launch ${label}: no compatible executable found in ${path.join(appPath, 'Contents', 'MacOS')}`, `Tried: ${executables.map((executable) => path.basename(executable)).join(', ')}. Install ${label}, reinstall it, or register a custom app path in ~/.webcmd/apps.yaml`);
    }
    throw lastMissingExecutableError ?? new CommandExecutionError(`Could not launch ${label}`, `Install ${label}, reinstall it, or register a custom app path in ~/.webcmd/apps.yaml`);
}
...
export async function resolveElectronEndpoint(site) {
    ...
    if (!app) {
        throw new CommandExecutionError(`No Electron app registered for site "${site}"`, 'Register the app in ~/.webcmd/apps.yaml or check the site name.');
    }
    ...
    if (process.platform !== 'darwin' && process.platform !== 'linux') {
        throw new CommandExecutionError(`${label} is not reachable on CDP port ${port}.`, `Auto-launch is not yet supported on ${process.platform}.\n` +
            ...
            `  • Set WEBCMD_CDP_ENDPOINT=http://127.0.0.1:${port}\n` +
    ...
    if (!appPath) {
        throw new CommandExecutionError(`Could not find ${label} on this machine.`, `Install ${label} or register a custom path in ~/.webcmd/apps.yaml`);
    }
    // Step 4: Launch
    //
    ...
    // webcmd launches because they all bundle a recent Chromium. Same
