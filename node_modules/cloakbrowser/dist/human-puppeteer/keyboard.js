 ...
 */
import { randRange, sleep } from '../human/config.js';
...
export async function humanType(page, raw, text, cfg, cdpSession) {
    const chars = [...text];
...
async function typeNormalChar(raw, ch, cfg) {
    await raw.down(ch);
    await sleep(randRange(cfg.key_hold));
    await raw.up(ch);
}
async function typeShiftedChar(raw, ch, cfg) {
...
async function typeShiftSymbol(page, raw, ch, cfg, cdpSession) {
    if (cdpSession) {
...
async function interCharDelay(cfg) {
