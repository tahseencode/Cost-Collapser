 ...
 */
export async function ensureBinary(licenseKey, browserVersion) {
...
function showWelcome(pro = false) {
    const marker = path.join(getCacheDir(), ".welcome_shown");
    if (!welcomeDue(marker, pro))
        return;
    console.error();
    ...
    if (pro) {
        ...
        console.error(`  Running free tier (v${freeMajor}). ` +
...
// Pro binary download
// ---------------------------------------------------------------------------
async function ensureProBinary(licenseKey, requestedVersion) {
    let version;
    if (requestedVersion) {
...
function maybeTriggerProUpdateCheck(licenseKey) {
    ...
    try {
        if (fs.existsSync(checkFile)) {
            ...
            if (Date.now() - lastCheck * 1000 < UPDATE_CHECK_INTERVAL_MS)
                return;
        }
    }
    catch {
